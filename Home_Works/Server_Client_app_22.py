...
def run_client(ip: str, port: int, msg: str):
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
        ...
        for line in msg.split(' '):
            print(f'..........Send data by client: {line}')
            sock.send(line.encode())
            response = sock.recv(1024)
            print(f'..........Response data by client: {response.decode()}')
    print(f'..........Data transfer completed by client')

if __name__ == '__main__':

    server_process = Process (target=run_server, args=('localhost', 4000, "OK")) #server initiation
    server_process.start()
    run_communication('localhost', 3000) #client initiation
